...
class GridDetector:
    ...
    def detect(self, frame, num_rows=5, num_cols=5):
        ...
        # The right 55% of the table holds the 5 checkbox columns; top 15% is header
        grid_start_x = tx + int(tw * 0.45)
        grid_w = (tx + tw) - grid_start_x
        grid_start_y = ty + int(th * 0.15)
        ...
        for r in range(num_rows):
            ...
            for c in range(num_cols):
                ...
                mx, my = int(cell_w * 0.20), int(cell_h * 0.20)
    ...
    def _find_table_roi(self, thresh, w, h):
        ...
        horiz = cv2.morphologyEx(thresh, cv2.MORPH_OPEN, cv2.getStructuringElement(cv2.MORPH_RECT, (int(w / 30), 1)))
        vert = cv2.morphologyEx(thresh, cv2.MORPH_OPEN, cv2.getStructuringElement(cv2.MORPH_RECT, (1, int(h / 30))))
